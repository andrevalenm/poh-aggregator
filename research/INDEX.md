...
Master index of all research in this repo. **~26,790 lines across 46 files**, written 2026-07-24 and after
...
| [world-verification-term-timeline.md](protocols/world-verification-term-timeline.md) | World ID — which of the AddressBook's terms wrote a `verifiedUntil` | The same premise as PoH's, one protocol over, and this one publishes too. `verifiedUntil - verificationLength()` reads the term at **head** while the entry was written in the past, and `setVerificationLength` is `onlyOwner`, touches no stored expiry, and cannot be renounced (`renounceOwnership` reverts) — so one owner transaction re-dates the whole book at once, and on a `Decay` curve a shortened term makes every World credential look uniformly fresher. It emits `VerificationLengthUpdated`; the constructor emits its own term as well, so unlike PoH **every era of this timeline has a published term** and no cohort can ever be lost to an unrecoverable first era. Full-history sweep, 2,711,105 → 32,843,977: **two governance logs in the contract's life** — the constructor and one router swap — and **zero** `VerificationLengthUpdated`, ever. The interesting measurement is the endpoint: Tenderly answers this full range with HTTP 200 and a *silently incomplete, non-deterministic* subset (`[24251140]`, `[2711105]` and `[]` from identical queries; an unfiltered call returned 980 logs all from the last 2,046 blocks), while 16M/8M/4M/2M chunks each return the complete set repeatedly. Hence two guards: the constructor's log must be present in its own block, and the newest term must explain head. +~230 ms once per process, and only for a subject who has an entry at all |
| [holonym-signed-not-proven.md](protocols/holonym-signed-not-proven.md) | Holonym / Human ID — what the Hub actually checks, and who is allowed to say a credential exists | The term audit's third mechanism, and the question was the wrong shape: the one-year ceiling is not a contract field an owner could change, it is **not on the contract at all**. `Hub.setSBT` runs no proof verification — it `ecrecover`s a signature over its own arguments against one stored address, so `circuitId`, the issuer in `publicValues[4]` and the expiry are all fields one off-chain service chose and signed. Our write-up and our caveat both said a circuit on chain enforced the ceiling; corrected. The key is `0x656D1dfb…Da01`, established from storage slot 8 (layout proved four ways: slot 0 == `owner()`, slot 1 == `"Holonym V3"`, slot 9 == the 238,713 token counter) and **recovered from all 76 mints in 150,000 blocks**, every one signed by it. `changeVerifier` is `onlyOwner`, has **no getter and emits nothing**, so a rotation is invisible to every indexer — swept from the constructor to head across 19 blocks and it has never moved. The hole is real and stated: with no event, a change made and reverted between two samples leaves no trace, though one that *stuck* is dated exactly (validated against slot 0, which did change, six seconds after deployment). The ceiling is unprovable on chain but **falsifiable**: `iat <= mint`, so `expiry - mintTimestamp` above 365 days would prove it exceeded — largest of 76 is **364.969 days**, 45 minutes below. +~360 ms once per process, and only for a subject who holds a credential |
| [holonym-issuer-pin.md](protocols/holonym-issuer-pin.md) | Holonym / Human ID — the issuer pin, and the person it refuses | `publicValues[4]` is the only thing separating a real credential from one somebody signed for themselves under the same circuit id, and the two keys we pin were **transcribed from Holonym's repositories** — nothing on chain declares them. Both failure directions checked. Too narrow was the live one and it was **silent**: a subject holding an SBT under an unrecognised issuer got `held: false` with no note and no caveat, so a refused credential and no credential looked identical from outside — and if the protocol rotates its key, the refusal is *ours*. Now `credential-issuer-not-recognised`, the second caveat in the codebase deliberately **not** filtered on `held`. The pin is also now measured rather than transcribed: ten windows from the deployment block (115,616,235) to head, every mint decoded from calldata, and **every `gov-id` mint in every era carries `0x03fae82f…1993` and every `biometrics` mint carries `0x0d4f849d…d922`** — a 200,000-block sweep at head (104 transactions) agrees exactly. The control is free: the two scored circuits carry different keys from each other and the unscored `phone` circuit a third, so the field discriminates and a match is information. At probe time a 30,000-block census of *live* credentials (one `eth_getLogs` per chunk + one `multicall`, ~450 ms, 9 holders / 10 credentials) says whether the pin is still the key in use. Two transport traps found and written down: viem's `getLogs` **action silently drops a caller's `topics`** (2.55.8) and answers with logs that do not match the filter — which had been quietly weakening two assertions in the live suite — and `multicall`'s `allowFailure` swallows a rate limit as per-call failures, so the endpoint rotation never fails over and a throttled node reads as an empty registry. +~50–220 ms cold, nothing warm, nothing for a subject holding no credential |
