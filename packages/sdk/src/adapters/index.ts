...
// Split out on the ax41 line: Circles stop-state, Coinbase's own on-chain indexer, and the
// term/expiry readers that let as-of see a credential the subject has since lost.
import { readCirclesStopped, type CirclesMintTime } from './circles.ts'
// The PoH term machinery, imported exactly as the ax41 line does — these all live in
// poh-term.ts, which is what makes cross-chain term resolution possible.
import {
  assumedTermHistory,
...
// NOT re-exported: ./coinbase.ts is the ax41 line's on-chain Coinbase reader and duplicates
// symbols that ./eas.ts already exports (EAS_ABI, COINBASE_VERIFIED_ACCOUNT_SCHEMA). The eas.ts
// implementation is the one wired into defaultAdapters, so coinbase.ts stays available to
// import directly — and keeps its own tests — without creating an ambiguous re-export here.
...
export function defaultAdapters(opts?: { subgraphUrl?: string }): AdapterProbe[] {
  return [
    ...
    pohV1Adapter(),
    selfAdapter(),
    // idenaAdapter(), brightIdAdapter() and civicPassAdapter() are deliberately NOT in the
    // default set — brightid is readable but frozen, civic-pass was discontinued 2025-07-31.
    // The ontology marks idena
    // `live: false` on scale grounds — 124 validated identities at epoch 214, down from 21,187
    // at peak — so scoring zeroes it anyway, and probing it would spend a network round-trip to
    // learn nothing. The module and its tests stay, for callers who want the read.
    humanodeAdapter(),
